...
>(
  ...
  >(
    ...
    if (process.env["NODE_ENV"] !== "production") {
      if (!isBrowser) {
        console.log(
          `[@olmokit/utils:storage] called 'get' outside of browser with default value '${JSON.stringify(
            defaultValue
          )}'.`
        );
      }
    ...
    if (isBrowser) {
      ...
      if (stored) {
        ...
        try {
          ...
          // if (process.env["NODE_ENV"] !== "production") {
          //   console.warn(
          //     `[@olmokit/utils:storage]: 'get' failed to parse stored value as JSON. Plain '${stored}' value is returned.`
          //   );
          // }
        }
      }
  ...
  >(
    ...
    if (process.env["NODE_ENV"] !== "production") {
      if (!isBrowser) {
        console.log(
          `[@olmokit/utils:storage] called 'set' outside of browser does not work.`
        );
      }
    }
    ...
    if (isBrowser) {
      try {
        ...
        if (process.env["NODE_ENV"] !== "production") {
          console.warn(`[@olmokit/utils:createStorage]: 'set' error.`, _e);
        }
      }
    }
  };
  ...
  const remove = <TKey extends Extract<keyof TConfig, string>>(key: TKey) => {
    if (process.env["NODE_ENV"] !== "production") {
      if (!isBrowser) {
        console.log(
          `[@olmokit/utils:storage] called 'remove' outside of browser does not work.`
        );
      }
    }
    ...
    if (isBrowser) {
      try {
        ...
        if (process.env["NODE_ENV"] !== "production") {
          console.warn(`[@olmokit/utils:createStorage]: 'remove' error.`, _e);
        }
      }
    }
  };
  ...
  >(
    ...
    if (process.env["NODE_ENV"] !== "production") {
      if (!isBrowser) {
        console.log(
          `[@olmokit/utils:storage] called 'has' outside of browser with default value '${JSON.stringify(
            defaultValue
          )}'.`
        );
      }
