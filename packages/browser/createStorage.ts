  ...
  return {
    ...
    getAll(defaultValues?: Partial<T>): T {
      if (!isBrowser) {
        if (process.env["NODE_ENV"] !== "production") {
          console.log(
            `[@olmokit/utils:createStorage] attempt to use 'getAll' outside of browser.`
          );
        }
        return {} as T;
      }
    ...
    setMany(newValues: Partial<T>) {
      if (process.env["NODE_ENV"] !== "production") {
        if (!isBrowser) {
          console.log(
            `[@olmokit/utils:createStorage] attempt to use 'setMany' outside of browser.`
          );
        }
      }
      if (isBrowser) {
    ...
    clear() {
      if (process.env["NODE_ENV"] !== "production") {
        if (!isBrowser) {
          console.log(
            `[@olmokit/utils:createStorage] attempt to use 'clear' outside of browser.`
          );
        }
      }
    ...
    watch: <TKey extends keyof T>(
      ...
      if (!isBrowser) {
        if (process.env["NODE_ENV"] !== "production") {
          console.log(
            `[@olmokit/utils:createStorage] attempt to use 'watch' outside of browser.`
          );
        }
        return noop;
      }
