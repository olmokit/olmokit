...
class AuthApi
{
  ...
  {
    return rtrim(config('env.APP_URL'), '/') . formatUrlPath("/_/auth/$type");
  }
  ...
  public static function getForm(string $type)
  {
    ...
    $requestUrl = self::getEndpointUrl('forms/' . $locale . '/' . $endpoint);
  ...
  public static function passwordRecovery(array $postData = [])
  {
    ...
    $postData = self::transformPostData($postData, false);
    $postData['reset_url'] = self::getRouteUrl('password-reset') . '?token=';
    // self::debugRequest($endpoint, $postData);
    $response = Http::post($endpoint, $postData);
  ...
  public static function startGuestSession()
  {
    ...
    if (!$guestToken) {
      Session::put(self::SESSION_GUEST_TOKEN, md5(time() . rand(100, 999)));
      Session::save();
    }
  }
  ...
  public static function requestWithAuth(
    ...
    if ($headers = self::getHeaders($allowGuest)) {
      if ($method === 'get') {
        $response = Http::withHeaders($headers)->get($url, $requestBody);
      }
      if ($method === 'post') {
        $response = Http::withHeaders($headers)->post($url, $requestBody);
      }
      if ($method === 'patch') {
        $response = Http::withHeaders($headers)->patch($url, $requestBody);
      }
      if ($method === 'put') {
        $response = Http::withHeaders($headers)->put($url, $requestBody);
      }
      if ($method === 'delete') {
