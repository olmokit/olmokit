...
class Helpers
{
  ...
  public static function getVersionedKey(string $input = '')
  {
    $ver = class_exists('\Composer\InstalledVersions')
      ? \Composer\InstalledVersions::getVersion('olmo/laravel-frontend') ?? ''
      : '';
   ...
   * @return string
   */
  public static function routeFilename(string $route = '', $srcFilename): string
  {
  ...
  public static function getRoutesMap()
  {
    ...
    foreach ($routesMap as $routeId => $routeInfo) {
      ...
      $addLocale = $enforceLocalisedUrls ? true : count($routeInfo['slug']) > 1;
      $rootRoute = false;
      ...
      foreach ($routeInfo['slug'] as $locale => $slug) {
        ...
        if (isset($class::$slug) && isset($class::$slug[$locale])) {
          $routeInfo['slug'] = array_merge($routeInfo['slug'], $class::$slug);
        }
  ...
  public static function apiGet(
    ...
    if (config($configEnvCacheKey) && $cacheKey && Cache::has($cacheKey)) {
      ...
      if (!$response) {
        ...
        if ($cacheKey) {
          $cacheTags = is_array($cacheTags) ? $cacheTags : $cacheTags($data);
