...
class Img
{
  ...
  private static function getParamsAsUrlSegments(array $params = [])
  {
    ...
    $compression = isset($params['compression']) ? $params['compression'] : '';
  ...
  private static function getFilename($src)
  {
    ...
    return Str::slug(preg_replace('/(.+)(\_[0-9]+)\.*$/i', '$1', $filename));
  }
  ...
  public function createImage()
  {
    ...
    if ($ext === 'gif' || $params['original']) {
      ...
      if (!$isPlaceholder) {
        ...
        Cache::tags([CacherTags::img])->put(
          $cacheKey . '-webp',
          $image->limitColors(null)->encode('webp', $params['compressionWebp'])
            ->encoded
        );
  ...
  public static function getPlaceholder()
  {
    ...
    try {
      $placeholder = Image::make(public_path('assets/images/placeholder.png'));
    } catch (NotReadableException $e) {
